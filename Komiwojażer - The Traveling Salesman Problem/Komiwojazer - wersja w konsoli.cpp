...
void wypiszCykl(vector<Wierzcholek> cykl) {
    for(int i = 0; i < cykl.size()-1; ++i) {
        cout << cykl[i].numer << "(" << cykl[i].x << ", " << cykl[i].y << ") -> " << cykl[i+1].numer << "(" << cykl[i+1].x << ", " << cykl[i+1].y << ")" << endl;
    }
}
...
int main() {
    ...
    for(int i = 0; i < iloscDanych; ++i) {
        Wierzcholek tempW;
        cin >> tempW.numer >> tempW.x >> tempW.y;
    ...
    cout << "Po zastosowaniu poprawek dlugosc cyklu to: " << dlugoscCyklu << endl;
